$(document).ready(function(){
	for(var key in data){
		...
		player_stats = player_stats.replace('assists', 'Asts');
		// console.log(urls);
		for(var temp in urls){
			// if((urls[temp]['URL'])
			var pos = urls[temp]['URL'].lastIndexOf('/');
			var length = urls[temp]['URL'].length;
			var newString = urls[temp]['URL'].substring(pos+1,length);
			newString = newString.replace('-', ' ');	
			newString = newString.replace('-', ' ');
			// console.log(data[key]['athlete_name'] + " " + newString);	
			if(newString.toLowerCase() == removeDiacritics(data[key]['athlete_name']).toLowerCase()){
				url = urls[temp]['URL'];
				console.log(url);
				break;
			}
		}
		var row = "<tr><td>" + data[key]['sport'] + "</td><td><a href='" + url + "'>" + data[key]['athlete_name'] + "</a></td><td>" + data[key]['athlete_team'] + "</td><td>" + data[key]['position'] + "</td><td>" + data[key]['athlete_points'] + "</td><td>" + player_stats+ "</td><td>" + "</td></tr>";
